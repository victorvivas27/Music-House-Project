...
export const getInstrumentAvailability = async (idInstrument, startDate, endDate) => {
  try {
    const response = await axios
      .get(`${BASE_URL}/available-dates/find/all/${startDate}/between/${endDate}/${idInstrument}`);
...
export const getAllAvailableDatesByInstrument = async (idInstrument) => {
  try {
    const response = await axios
      .get(`${BASE_URL}/available-dates/${idInstrument}`);
    return response.data;
  } catch (error) {
    handleApiError(error);
  }
...
export const addAvailableDates = async (availableDates) => {
  try {
    const response = await axios
      .post(`${BASE_URL}/available-dates`, availableDates);
    return response.data;
  } catch (error) {
    handleApiError(error);
  }
